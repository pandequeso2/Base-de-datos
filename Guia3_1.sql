...
DECLARE
    ...
    v_rut       Empleado.numrut_emp%TYPE := &Ingrese_Rut;
    v_dvrut     VARCHAR2(1);
    v_sueldo    Empleado.sueldo_emp%TYPE;
    v_bono      NUMBER(10);
BEGIN
    SELECT nombre_emp, numrut_emp, dvrut_emp, sueldo_emp,appaterno_emp,apmaterno_emp
    INTO v_nombre, v_rut, v_dvrut, v_sueldo, v_appaterno,v_apmaterno
    FROM Empleado
    WHERE NUMRUT_emp = v_rut; -- v_nombre aquí contiene el valor de búsqueda

    -- Calcular la bonificación
    v_bono := v_sueldo * 0.40; -- 40% del sueldo
    ...
    DBMS_OUTPUT.PUT_LINE('RUN: ' || v_rut || '-' || v_dvrut);
    DBMS_OUTPUT.PUT_LINE('Sueldo: ' || v_sueldo);
    DBMS_OUTPUT.PUT_LINE('Bonificación: ' || (v_porc*v_sueldo)); -- Mostrar la bonificación calculada

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Error: No se encontró el empleado con el nombre: ' || v_nombre);
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error, el error es: ' || SQLERRM);
END;
--Caso 2
